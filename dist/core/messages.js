export class MessageManager {
    ...
    send(params) {
        ...
        if (!sender)
            throw new Error(`Sender agent '${params.from}' not found in project '${params.projectSlug}'`);
        const recipientIds = [];
        for (const recipientName of params.to) {
            const recipient = this.agents.whois(params.projectSlug, recipientName);
            if (!recipient)
                throw new Error(`Recipient agent '${recipientName}' not found`);
    ...
    `);
        const txn = this.db.instance.transaction(() => {
            const msg = insertMsg.get(sender.projectId, threadId, sender.id, params.subject, params.body, params.importance || 'normal', params.ackRequired ? 1 : 0);
    ...
    fetchInbox(projectSlug, agentName, limit = 20) {
      ...
      LIMIT ?
    `);
        const rows = stmt.all(agent.id, limit);
        return rows.map(r => this.mapRowToMessage(r, r.sender_name));
    }
    fetchThread(projectSlug, threadId) {
        const stmt = this.db.instance.prepare(`
      SELECT m.*, a.name as sender_name
      FROM messages m
      JOIN agents a ON m.from_agent_id = a.id
    ...
    `);
        const rows = stmt.all(projectSlug, threadId);
        return rows.map(r => this.mapRowToMessage(r, r.sender_name));
    }
    mapRowToMessage(row, senderName) {
        return {
            id: row.id,
